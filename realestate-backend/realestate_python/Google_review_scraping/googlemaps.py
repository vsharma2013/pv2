...
class GoogleMapsScraper:
    ...
    def get_count_reviews(self):
        count_review_text = self.driver.find_element_by_xpath('//div[contains(@class, "section-layout-root")]//div[@class="gm2-caption"]').text
        count_review = re.search(r'(.*?) ', count_review_text).group(1)
        return count_review
...
##        id_review = review.find('button', class_='section-review-action-menu')['data-review-id']
        ...
        photos_soup = review.find_all('button', class_='section-review-photo')
        photos_urls = ''
        for p in photos_soup:
            random_id = uuid.uuid4()
            image_path = 'data/photos/'+str(random_id)+'.jpg'
            image_path_in_excel = 'photos/'+str(random_id)+'.jpg'
            style = str(p['style'])
            try:
                photo_url = re.search(r'background-image: url\("(.*?)"\)', style).group(1)
            except:
                photo_url = re.search(r'background-image:url\((.*?)\)', style).group(1)
            req_image = requests.get(photo_url)
            if req_image.status_code == 200:
                with open(image_path,"wb") as f:
                    f.write(req_image.content)
            else:
                print(photo_url, '=> Failed to download image')
            photos_urls += image_path_in_excel + ", "
        photos_urls = photos_urls[0:-2]
        item['photos_path'] = photos_urls
        return item
    ...
    def __expand_reviews(self):
        ...
        for l in links:
            l.click()
        time.sleep(2)
        photos_links = self.driver.find_elements_by_xpath('//div[@class="section-review-photo-overlay"]')
        for li in photos_links:
            li.click()
        time.sleep(2)
